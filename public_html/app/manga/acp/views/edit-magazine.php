...
$id = (int)$_GET['id'];
$thisMagazine = $db->Query(APP_TABLES_PREFIX.'manga_magazines', '*', array('id' => $id));

?>
<div class="col-lg-8">
	<div class="panel panel-default">
		<div class="panel-heading">
			<h3 class="panel-title">Edit Magazine</h3>
		</div>
		<div class="panel-body">
			<div id="UserTableContainer" style="width: 100%;">
				<div id="addManga_output"></div>
				<form id="addManga_form" role="form" method="POST" action="../app/manga/controllers/cont.editMagazine.php">
					<input type="hidden" name="id" id="inputId" class="form-control" value="<?=$_GET['id']?>">
					<div class="form-group">
						<label for="exampleInputEmail1"><?=$lang['Name']?></label>
						<input type="text" class="form-control" id="name" name="name" value="<?=$thisMagazine[0]['name']?>">
					</div>
					<div class="form-group">
						<label for="exampleInputEmail1"><?=$lang['Slug']?></label>
						<div class="form-group">
							<input type="text" class="form-control" id="magazineSlug" name="slug" value="<?=$thisMagazine[0]['slug']?>">
						</div>
					</div>
					<div class="form-group">
						<label for="exampleInputEmail1"><?=$lang['Content']?></label>
						<textarea class="form-control" name="content" id="content" cols="100%" rows="8" placeholder="Description" value="<?=$thisMagazine[0]['content']?>"></textarea>
						<?=$lang['Content_ex']?>
					</div>
