...
if($_GET['type'] == 'chapter' && $_SESSION['thisUser']['role'] == 2 || $_GET['type'] == 'chapter' && $_SESSION['thisUser']['role'] == 3){ 
	...
	if($_GET['mslug']) {
		...
		if ($checkUserGroup || $checkSubmitter) {
						...
						<div id="UserTableContainer" style="width: 100%;">
							<form id="addManga_form" role="form" method="POST" action="/app/manga/controllers/cont.user.addChapter.php">
								...
								<div class="form-group">
									...
									<input type="text" id="chapterno" class="form-control" name="chapter" placeholder="Ex: 78">
								</div>
								<div class="form-group">
									<label for="exampleInputEmail1"><?=$lang['Name']?></label>
									<input type="text" class="form-control" name="name" placeholder="Ex: The Last Game">
								</div>
								<div class="form-group">
			...
			<?php
			$user->ajaxForm('addManga','/manage/list-chapter/'.$_GET['mslug'].'.html');
		} else {
			$user->alert('danger', 'You can not add chapter!');
		}
	}
}
echo '</div>';
