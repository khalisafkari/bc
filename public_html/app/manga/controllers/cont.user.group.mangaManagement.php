...
try
{
	...
	if($_GET["action"] == "list")
	{
		...
		$isGroup = $db->Query(APP_TABLES_PREFIX.'user','group_uploader',array('id'=>$_SESSION['userId']));
 		$Group = $isGroup[0]['group_uploader'];
		$result = mysqli_query($db->Connect(),"SELECT COUNT(id) AS RecordCount FROM ".APP_TABLES_PREFIX."manga_mangas WHERE group_uploader = ".$Group." AND group_uploader != 0 AND name LIKE '%".$_POST['name']."%' ".$check.";");
		$row = mysqli_fetch_assoc($result);
		$recordCount = $row['RecordCount'];

		//Get records from database
		$result = mysqli_query($db->Connect(),"SELECT * FROM ".APP_TABLES_PREFIX."manga_mangas WHERE group_uploader = ".$Group." AND group_uploader != 0 AND name LIKE '%".$_POST['name']."%' ".$check." ORDER BY " . $_GET["jtSorting"] . " LIMIT " . $_GET["jtStartIndex"] . "," . $_GET["jtPageSize"] . ";");
