...
$string = $huy->curl($url);
preg_match_all('#id="tables"><a href="(.*)".*>(.*)</a>#imsU', $string, $listManga);
$list_manga = array_combine($listManga[1], $listManga[2]);
//$list_manga = ['manga-vallow-of-rebellion-manga-raw.html' => 'Vallow of Rebellion (Manga) - Raw'];
if (MANGA) {
	foreach ($list_manga as $aUrl => $aName) {
        $html = $huy->curl('https://lhnovels.net/'.$aUrl);
		...
		if (count($checker)) {
			...
			foreach ($arr as $url_chapter => $value) {
				$html = $huy->curl(trim('https://lhnovels.net/'.$url_chapter));
			...
			if (count($arr)) {
				...
				if ($result_2) {
					...
					foreach($whoBookmark as $one){
						$db->Create(APP_TABLES_PREFIX . 'manga_notification', array('type'=>2,'mid'=>$mid, 'cid'=>$cid, 'name'=>$thisManga[0]['name'], 'slug'=>$slug, 'url'=>'/'.$mid.'/'.$cid.'/','chapter'=>$chap_max,'content'=>'The Manga '.$thisManga[0]['name'].' you are following has a new . '.$chap_max.' mới', 'user'=>$one['user'], 'time'=>$now));
					}
		...
		$manga['magazine'] = !empty($magazine[1]) ? trim(implode(',', $magazine[1])) : '';
		...
		if (preg_match('#On Going#is', $m_status[1])) {
			...
			$query = 1;
			$mid = $query = $db->Create(APP_TABLES_PREFIX.'manga_mangas',array('name'=>$manga['name'],'slug'=>$manga['slug'],'m_status'=>$manga['status'],'authors'=>$manga['authors'],'artists'=>$manga['artists'],'trans_group'=>$manga['trans_group'],'genres'=>$manga['genres'],'magazine'=>$manga['magazine'],'other_name'=>$manga['other_name'],'description'=>$manga['description'],'submitter'=>$manga['submitter'],'cover'=>$manga['cover'],'views'=>'1','last_update'=>$now));
			if ($query) {
